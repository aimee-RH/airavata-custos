 ...
 *
 * Returns complete login activity summaries with server-side search, ordering, and pagination.
 */
 ...
 *
 * Returns lifetime, current-month, and rolling-window login metrics plus a daily trend. Rolling and monthly boundaries use each daily fact's recorded user-local IANA timezone.
 */
...
/**
 * Get analytics for one selected user
 *
 * Returns login engagement metrics restricted to the explicitly selected user.
 */
...
/**
 * List inactive users (paginated)
 *
 * Returns users inactive for at least the requested number of user-local calendar days. Never-logged-in users are included first.
 */
export const getUsersInactive = <ThrowOnError extends boolean = false>(options?: Options<GetUsersInactiveData, ThrowOnError>): RequestResult<GetUsersInactiveResponses, GetUsersInactiveErrors, ThrowOnError> => (options?.client ?? client).get<GetUsersInactiveResponses, GetUsersInactiveErrors, ThrowOnError>({
    security: [{ name: 'Authorization', type: 'apiKey' }],
    url: '/users/inactive',
    ...options
});

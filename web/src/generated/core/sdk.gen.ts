...
/**
 * Record the caller's sign-in
 *
 * Records one sign-in for the authenticated caller. Identity and timing come from the verified OIDC token, never the request body, and the body must be empty or `{}`. Repeating the call for the same token is idempotent and answers 200 without changing any counts.
 */
export const postMeLoginEvents = <ThrowOnError extends boolean = false>(options?: Options<PostMeLoginEventsData, ThrowOnError>): RequestResult<PostMeLoginEventsResponses, PostMeLoginEventsErrors, ThrowOnError> => (options?.client ?? client).post<PostMeLoginEventsResponses, PostMeLoginEventsErrors, ThrowOnError>({
    security: [{ name: 'Authorization', type: 'apiKey' }],
    url: '/me/login-events',
    ...options
});
...
/**
 * Aggregate sign-in analytics for one OIDC-linked identity
 */
export const getUsersByIdActivityAnalytics = <ThrowOnError extends boolean = false>(options: Options<GetUsersByIdActivityAnalyticsData, ThrowOnError>): RequestResult<GetUsersByIdActivityAnalyticsResponses, GetUsersByIdActivityAnalyticsErrors, ThrowOnError> => (options.client ?? client).get<GetUsersByIdActivityAnalyticsResponses, GetUsersByIdActivityAnalyticsErrors, ThrowOnError>({
    security: [{ name: 'Authorization', type: 'apiKey' }],
...
/**
 * List sign-in activity for OIDC-linked identities
 *
 * One row per OIDC-linked identity. `window` and `status` are applied before pagination, and the response echoes both so a caller can detect an unfiltered answer. Statuses are disjoint: active has signed in within the window, dormant has signed in earlier, never has no recorded sign-in.
 */
export const getUsersActivity = <ThrowOnError extends boolean = false>(options: Options<GetUsersActivityData, ThrowOnError>): RequestResult<GetUsersActivityResponses, GetUsersActivityErrors, ThrowOnError> => (options.client ?? client).get<GetUsersActivityResponses, GetUsersActivityErrors, ThrowOnError>({
    security: [{ name: 'Authorization', type: 'apiKey' }],
    url: '/users/activity',
    ...options
});
...
/**
 * Aggregate sign-in analytics for all OIDC-linked identities
 *
 * Population-wide engagement over the requested window. Supplemental fields (`prior_active_users`, `dormant_over_90_days`, `oldest_never_created_at`) may be null when the population cannot produce them.
 */
export const getUsersActivityAnalytics = <ThrowOnError extends boolean = false>(options?: Options<GetUsersActivityAnalyticsData, ThrowOnError>): RequestResult<GetUsersActivityAnalyticsResponses, GetUsersActivityAnalyticsErrors, ThrowOnError> => (options?.client ?? client).get<GetUsersActivityAnalyticsResponses, GetUsersActivityAnalyticsErrors, ThrowOnError>({
    security: [{ name: 'Authorization', type: 'apiKey' }],
